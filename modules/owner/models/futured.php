...
class futured extends Booking
{
    ...
    public function search($params)
    {
        $query = Booking::find()->where(['>=', 'from', date("Y-m-d")])->andWhere(['cancel' => 0])
                    ->andWhere(['object_id' => Yii::$app->request->get('id')])->orderBy('from ASC');
        ...
        if (empty($this->from) && empty($this->to) && !empty($this->id)) {
            $bookid = [];
            $bookings = Booking::find()->where(['cancel' => 0,'object_id' => Yii::$app->request->get('id')])->all();
            ...
            $bookid = [];
            $bookings = Booking::find()->where(['cancel' => 0,'object_id' => Yii::$app->request->get('id')])->andWhere(['>=','from',$this->from])->all();
            foreach ($bookings as $booking) {
                if (empty($this->id)) {
                    $bookid []= $booking->id;
            ...
            $bookid = [];
            $bookings = Booking::find()->where(['cancel' => 0,'object_id' => Yii::$app->request->get('id')])->andWhere(['<=','to',$this->to])->all();
            foreach ($bookings as $booking) {
                if (empty($this->id)) {
                    $bookid []= $booking->id;
                } elseif (mb_stripos($booking->name, $this->id) !== false || mb_stripos($booking->object_id . '-' . $booking->id,$this->id) !== false) {
            ...
            $bookid = [];
            $bookings = Booking::find()->where(['cancel' => 0,'object_id' => Yii::$app->request->get('id')])->andWhere(['between','from',$this->from,$this->to])->all();
            foreach ($bookings as $booking) {
                if (empty($this->id)) {
                    $bookid []= $booking->id;
                } elseif (mb_stripos($booking->name, $this->id) !== false || mb_stripos($booking->object_id . '-' . $booking->id,$this->id) !== false) {
