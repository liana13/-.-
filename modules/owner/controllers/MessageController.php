 ...
 */
class MessageController extends OwnerController
{
    /**
     * @inheritdoc
     */
    ...
    public function actionIndex()
    {
        $messages = Message::find()->where(['user_one'=>Yii::$app->user->getId()])->orWhere(['user_two'=>Yii::$app->user->getId()])
                    ->orderBy('id DESC')->all();
        if (count($messages)!=0) {
            $dialogue = Message::find()->where(['user_one'=>Yii::$app->user->getId()])->andWhere(['statusowner' => null])
                        ->orWhere(['user_two'=>Yii::$app->user->getId()])->andWhere(['statusowner' => null])
                        ->groupBy('dialogue_id')->orderBy('id DESC')->all();

            $unread = Message::find()->where(['user_two'=>Yii::$app->user->getId()])->andWhere(['status' => 0])->andWhere(['statusowner' => null])
                        ->groupBy('dialogue_id')->orderBy('id DESC')->all();
        } else {
            $dialogue = [];
            $unread = [];
    ...
    public function actionCreate()
    {
        ...
        if ($model->load(Yii::$app->request->post()) && $model->save()) {
            $useremail = User::findOne(['id'=>$model->user_two])->email;
            $messageurl = Yii::$app->urlManager->createAbsoluteUrl(['/cabinet/message']);
            Yii::$app->mailer->compose()
                ->setTo($useremail)
                ->setFrom(['noreply@'.Yii::$app->request->serverName=>Yii::$app->name])
                ->setSubject('Новое сообщение в ' ." " .Yii::$app->name)
                ->setHtmlBody('Здравствуйте, Вам поступило сообщение от администратора объекта размещения. Чтобы увидеть сообщение и ответить на него пожалуйста, перейдите в свой личный кабинет на портале '.Yii::$app->name.' в раздел <a href="'. $messageurl.'">«Сообщения»</a>.')
                ->send();
