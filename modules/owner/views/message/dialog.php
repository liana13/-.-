...
$this->title = "Переписка с ".$collocutor->username." (Объект - ".$service." ".$object->title." ".$locality.")";
$this->params['breadcrumbs'][] = ['label' => 'Moи сообщения', 'url' => ['index']];
    ...
    <div class="shadow-out">
        <?php foreach ($messages as $message): ?>
            ...
            <?php elseif($message->user_one == Yii::$app->user->getId()): ?>
                <div class="event-msg">
                    <div class="event-info">
                        <span class="event-name"><?=$service." ".$object->title." ".$locality?></span>
                        <div class="event-sent hidden-xs">
                            <small>Отправлено: <?=explode(' ', $message->created_at)[0]?></small>
                        </div>
            ...
            <?php endif; ?>
        <?php endforeach; ?>
        <?php $form = ActiveForm::begin(['id' => 'message-form', 'action' => ['/owner/message/create']]); ?>
